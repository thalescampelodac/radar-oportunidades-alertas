...
exports.getBrazilianStocksFundamentals = getBrazilianStocksFundamentals;
exports.getBrazilianStocksWithFallback = getBrazilianStocksWithFallback;
exports.getAvailableBrazilianTickers = getAvailableBrazilianTickers;
const radar_storage_1 = require("./radar-storage");
...
function mapBrapiResult(result) {
    const ticker = result.symbol?.toUpperCase();
    if (!ticker) {
        return null;
    }
    const dy = normalizePercent(result.defaultKeyStatistics?.dividendYield) ??
    ...
    return {
        ...
        liquidez: normalizeNumber(result.financialData?.currentRatio) ??
...
function getBrapiRequestTickers(requestedTickers, hasToken) {
    if (hasToken) {
...
async function fetchFromBrapi(tickers) {
    ...
    try {
        console.log(`[StockData] Consultando brapi para ${requestTickers.join(", ")} | token configurado: ${token ? "sim" : "nao"}`);
...
function getFallbackStocks() {
    try {
        const radar = (0, radar_storage_1.readRadar)();
...
function mergeStocks(baseStocks, liveStocks) {
...
async function getBrazilianStocksWithFallback(tickers) {
    ...
    console.log(`[StockData] Merge concluído: ${liveStocks.length} ativos reais + ${baseStocks.length} de fallback`);
    return mergedStocks;
}
function getAvailableBrazilianTickers() {
